...
/* END */
  ...
  public function cancelOrder(request $request){
        ...
        CsTransactions::where('trans_id',$request->trans_id)->where('trans_user_id',$rowUserData->user_id)->update(['trans_status'=>5,'trans_cancel_reason'=>$request->order_reason,'trans_cancellation_date'=>date('Y-m-d')]);
        $aryResponse['status']="success";
            $aryResponse['notification'] ="Order cancelled successfully";
            try {
                $rowOrderData = CsTransactions::where('trans_id',$request->trans_id)->where('trans_user_id',$rowUserData->user_id)->first();
